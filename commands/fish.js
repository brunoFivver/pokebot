...
module.exports.run =  async (bot,msg, args, prefix,database) => {
  if(cooldown.has(msg.author.id)) return msg.reply(`Hey You are in a cooldown,you need to wait 2 minutes in order to use this command again!`)
    ...
    database.ref(`Users/${msg.author.id}/Equipaments/fishing_rod`).once("value",async function(shovel) {
      ...
      if(shovel.val() === null) {
        ...
        .setColor('GREY')
        .setTitle(`You don't have a fishing_rod To fish`)
        .setDescription("`Use p!shop buy equipments fishing_rod 1` In order to get a fishing_rod!")
        .setTimestamp()      
        msg.reply(dig)        
      }else {
        database.ref(`Users/${msg.author.id}/Equipaments/fishing_rod`).update({
          durability: shovel.val().durability - 10
        })
        if(shovel.val().durability <= 0)  {
           msg.reply("Your fishing rod broke! Buy a new one from the shop! **p!shop buy equipments fishing_rod 1**")
           return database.ref(`Users/${msg.author.id}/Equipaments/fishing_rod`).remove()
        }
        P.getTypeByName("water")
    .then(async function(response) {
        var number = Math.floor(Math.random() * response.pokemon.length) + 1
        var poke = response.pokemon[number]
        if(!poke) return msg.reply("You  try to catch a pokémon, but he ate his bait and ran away, but no Pokémon was found! Try again!")
      ...
      if(pokemon === undefined) return msg.reply("You  try to catch a pokémon, but he ate his bait and ran away, but no Pokémon was found! Try again!")
        var image = ""
      ...
      .setColor('#ff0000')
        .setTitle(`You Fish a ${pokemon.name} | LVL ${level}`)
